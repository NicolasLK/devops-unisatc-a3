...
test.describe('Painel Administrativo Strapi', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('http://127.0.0.1:1337/admin/auth/login')
  ...
  test.describe('Dashboard e Navegação @admin @smoke', () => {
    test('deve carregar o dashboard corretamente', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="navbar"]').or(page.locator('nav'))).toBeVisible();
    ...
    test('deve navegar entre seções do admin', async ({ page }) => {
      ...
      await page.waitForLoadState('networkidle');
      await expect(page.locator('h1, h2').filter({ hasText: /Content Manager/i })).toBeVisible();
      
      // Testa navegação para Media Library
      await page.click('text=Media Library');
      await page.waitForLoadState('networkidle');
      await expect(page.locator('h1, h2').filter({ hasText: /Media Library/i })).toBeVisible();
  ...
  test.describe('Content-Type Builder @admin @content', () => {
    test('deve acessar Content-Type Builder', async ({ page }) => {
      ...
      await expect(page.locator('h1, h2').filter({ hasText: /Content-Type Builder/i })).toBeVisible();
      
      // Verifica botão para criar novo content type
      await expect(page.locator('text=Create new collection type').or(page.locator('button').filter({ hasText: /create/i }))).toBeVisible();
    });
  ...
  test.describe('Content Manager @admin @content', () => {
    ...
    test('deve criar novo conteúdo se content type existir', async ({ page }) => {
      await page.click('text=Content Manager');
      await page.waitForLoadState('networkidle');
      
      // Procura por algum content type existente
      const contentTypes = page.locator('[data-testid="collection-type"]').or(
        page.locator('li').filter({ hasText: /collection/i })
      );
      ...
      if (count > 0) {
        ...
        const createButton = page.locator('text=Create new entry').or(
  ...
  test.describe('Media Library @admin @media', () => {
    test('deve acessar Media Library', async ({ page }) => {
      ...
      await expect(page.locator('text=Add new assets').or(page.locator('input[type="file"]'))).toBeVisible();
    });
    ...
    test('deve mostrar interface de upload', async ({ page }) => {
      ...
      const addButton = page.locator('text=Add new assets').or(
      ...
      if (await addButton.isVisible()) {
        ...
        await expect(page.locator('input[type="file"]').or(page.locator('[data-testid="upload-area"]'))).toBeVisible();
      }
    });
  });
  ...
  test.describe('Settings @admin @settings', () => {
    ...
    test('deve acessar configurações de usuários e permissões', async ({ page }) => {
      await page.click('text=Settings');
      await page.waitForLoadState('networkidle');
      
      // Tenta clicar em Users & Permissions
      const usersPermissions = page.locator('text=Users & Permissions').or(
        page.locator('text=Roles').or(page.locator('text=Users'))
      );
      ...
      if (await usersPermissions.first().isVisible()) {
        ...
        await expect(page.locator('h1, h2').filter({ hasText: /(Users|Roles|Permissions)/i })).toBeVisible();
      }
    });
  });
  ...
  test.describe('Perfil do Usuário @admin @profile', () => {
    test('deve acessar configurações do perfil', async ({ page }) => {
      ...
      const userMenu = page.locator('[data-testid="user-menu"]').or(
        page.locator('button').filter({ hasText: new RegExp(process.env.ADMIN_EMAIL || 'admin', 'i') })
      ).or(
      ...
      if (await userMenu.isVisible()) {
        ...
        const profileLink = page.locator('text=Profile').or(page.locator('text=Perfil'));
        ...
        if (await profileLink.isVisible()) {
          ...
          await expect(page.locator('h1, h2').filter({ hasText: /(Profile|Perfil)/i })).toBeVisible();
        }
      }
    });
  });
  ...
  test.describe('Funcionalidades Gerais @admin @regression', () => {
    test('deve fazer logout corretamente', async ({ page }) => {
      ...
      if (await userMenu.isVisible()) {
        ...
        const logoutButton = page.locator('text=Logout').or(page.locator('text=Sair'));
